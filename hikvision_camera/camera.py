...
class Camera:
    ...
    def postOutputRequest(self, state) -> None:
        if state is 1:
            response = requests.put(url, data=self.raw_high_request, auth=HTTPDigestAuth(self.username, self.password))
        elif state is 0:
            response = requests.put(url, data=self.raw_low_request, auth=HTTPDigestAuth(self.username, self.password))
